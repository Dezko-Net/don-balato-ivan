...
async function main() {
  ...
  while (true) {
    ...
    total += docs.length;
    console.log(`Batch: ${docs.length} docs (total: ${total}/804)`);
    ...
    for (const doc of docs) {
      ...
      try {
        ...
        const updateRes = await fetch(updateUrl, {
          method: 'PATCH',
          headers: { ...headers, 'X-Appwrite-Response-Format': '1.6.0' },
          body: JSON.stringify({ data: { PRICE: newPrice, WHOLESALEPRICE: newWholesale } }),
        });
        if (updateRes.ok) {
          updated++;
          if (updated % 50 === 0) console.log(`Actualizados: ${updated}...`);
        } else {
          console.error(`Error en ${doc.$id}:`, updateRes.status, await updateRes.text());
        }
      } catch (e) {
