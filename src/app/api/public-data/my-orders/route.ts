...
async function findOrders(collectionId: string, userId: string, email: string) {
  let result = await serverListDocuments(collectionId, [q('equal', 'USERID', [userId]), orderDesc, limit]).catch(() => ({ documents: [], total: 0 }));
  if (!result.documents.length && email) {
    result = await serverListDocuments(collectionId, [q('equal', 'CUSTOMEREMAIL', [email]), orderDesc, limit]).catch(() => ({ documents: [], total: 0 }));
  }
