...
import {
  ...
  X, Check, Package, RefreshCw, Printer, AlertTriangle, CheckCircle, Zap, Image as ImageIcon
} from 'lucide-react';
...
export default function PosTerminalPage() {
  ...
  // Cargar catálogo de Appwrite a la memoria local (0ms scan speed)
  ...
  // Autofoco constante en el buscador/escáner
  useEffect(() => {
  ...
  // Manejo de escáner de código de barras (Enter)
  const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && search.trim()) {
      e.preventDefault();
      const q = search.trim().toLowerCase();
      // 1. Coincidencia exacta por código de barra o SKU
      ...
      // 2. Si hay 1 solo resultado filtrado por coincidencia parcial
  ...
  // Filtrado de catálogo para lista desplegable
  ...
  // Procesar Cobro e Impresión de Boleta Térmica
  const handleProcessSale = async () => {
    ...
    try {
      // 1. Descontar Stock en Appwrite Cloud para cada producto
      ...
      // 2. Preparar Datos de la Boleta Térmica
      ...
      // 3. Abrir Ventana de Impresión de Boleta
      openReceiptPrintWindow(receiptData);

      // 4. Limpiar Carrito y recargar catálogo
      setCart([]);
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans">
      ...
      <header className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center font-black text-xl shadow-lg shadow-emerald-500/20">
            Y
          </div>
          <div>
            <h1 className="text-xl font-black tracking-tight">Punto de Venta POS — Yaxsel</h1>
      ...
      <main className="flex-1 grid grid-cols-12 gap-6 p-6 min-h-0">
        ...
        {/* Columna Derecha: Carrito de Compras & Cobro */}
      ...
      {showCheckoutModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl space-y-6">
            ...
            {/* Confirmar Cobro e Imprimir Boleta */}
            <button
